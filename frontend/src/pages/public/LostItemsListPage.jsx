...
export default function LostItemsListPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await lostItemService.getAll(params);
  ...
  return (
    <div style={{ paddingTop: 'calc(var(--navbar-height) + 24px)', maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
      <div className="page-header">
        <h1 className="page-title">Lost <span>Items</span></h1>
      </div>

      <div className="search-filter-bar">
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: 8, flex: 1 }}>
          <input placeholder="Search lost items..." value={keyword} onChange={e => setKeyword(e.target.value)} style={{ flex: 1, maxWidth: 'none' }} />
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-3">
            {items.map(item => (
              <Link to={`/lost-items/${item.id}`} key={item.id} className="item-card">
                ...
                <div className="item-card-body">
                  ...
                  <div className="item-meta">
                    <span>📍 {item.locationLost || 'Unknown'}</span>
                    <span>📅 {formatDate(item.dateLost)}</span>
                  </div>
                  <p className="item-desc">{truncate(item.description, 80)}</p>
                  <div className="item-card-footer">
                    <StatusBadge status={item.status} />
                    {item.rewardAmount > 0 && <span style={{ color: 'var(--secondary)', fontWeight: 700 }}>₹{item.rewardAmount}</span>}
                  </div>
                </div>
              </Link>
            ))}
